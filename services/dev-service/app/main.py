...
def create_app() -> FastAPI:
    ...
    @app.get("/", tags=["system"])
    def root(): return success_response(data={"service": settings.app_name, "status": "ok", "docs_url": "/docs", "health_url": "/health", "ready_url": "/ready"})
    @app.get("/health", tags=["health"])
    def health_check(): return success_response(data={"status": "ok", "service": settings.app_name})
    @app.get("/ready", tags=["health"])
    def ready():
        try:
            with SessionLocal() as db: db.execute(text("SELECT 1"))
        except Exception:
            return JSONResponse(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content=error_response(message="Service is not ready.", code="service_unready"))
