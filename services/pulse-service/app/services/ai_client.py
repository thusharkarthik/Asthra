...
class AIClient:
    ...
    def complete(self, prompt: str, *, system_prompt: str | None = None, request_id: str | None = None) -> dict:
        if not settings.ai_features_enabled or not self.ai_service_url:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="AI features are disabled or AI_SERVICE_URL is not configured.")
        ...
        try:
            ...
            pass
        return {"current_situation": generated_text, "raw_response": generated_text}
