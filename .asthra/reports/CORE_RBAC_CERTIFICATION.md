...
Sidebar management now has a Core Navigation Registry primary source plus compatibility fallbacks:

- Primary source: `navigation` from `GET /context/platform`, generated by Core Navigation Registry and filtered by mode, feature flags, module availability, and backend permission codes.
- Compatibility source: `availableModules[]` from `GET /context/platform`, generated by Core Module Registry and filtered by feature flags, navigation mode, and backend permission codes.
- Frontend dynamic mapping: `frontend/src/lib/module-nav-registry.ts` converts backend navigation or module metadata into sidebar sections.
...
- Backend registry: `services/core-service/app/services/navigation_registry.py` defines code-owned nav items with `nav_key`, mode, group, route, icon, order, feature flag, module key, and any-of permission requirements.
...
No full sidebar customization was implemented in this batch. Follow-up should decide whether API Keys, Memory, Assistant, and generic Settings require explicit registry permissions instead of authenticated visibility. Future sidebar customization may hide or reorder items but must not grant access.
