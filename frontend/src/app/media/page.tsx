...
export default function MediaPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Media" description="Multimodal metadata layer for assets, transcripts, annotations, tags, and processing jobs." actions={<ModulePrimaryActions><Link className="inline-flex h-9 items-center rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground" href="/media/assets">Add Media Asset Metadata</Link><Link className="inline-flex h-9 items-center rounded-md border px-3 text-sm font-medium" href="/media/collections">Create Collection</Link><Link className="inline-flex h-9 items-center rounded-md border px-3 text-sm font-medium" href="/media/processing-jobs">Start Processing Job</Link></ModulePrimaryActions>} /><ModuleSubnav items={mediaNavItems} activePath={pathname} />{!workspaceId ? <PlatformSetupGuide moduleName="Media" hasWorkspace={false} /> : <><ModuleStatsGrid stats={[{ title: "Media Assets", value: (assets.data ?? []).length, description: "Tracked media metadata" }, { title: "Collections", value: (collections.data ?? []).length, description: "Organized asset groups" }, { title: "Processing Jobs", value: (jobs.data ?? []).length, description: "OCR/transcription placeholders" }, { title: "Transcripts", value: "Ready", description: "Transcript metadata surface" }, { title: "Failed Jobs", value: (jobs.data ?? []).filter((job) => job.status === "failed").length, description: "Processing jobs needing review" }]} /><div className="grid gap-4 lg:grid-cols-2"><ModuleDashboardCard title="Asset Gallery">{(assets.data ?? []).length === 0 ? <EmptyState title="No assets yet" /> : <div className="space-y-3">{(assets.data ?? []).map((asset) => <Link key={asset.id} className="block rounded-md border p-3 hover:bg-muted/60" href={`/media/assets/${asset.id}`}><div className="font-medium">{asset.title}</div><div className="mt-2"><MediaTypeBadge value={asset.asset_type} /></div></Link>)}</div>}</ModuleDashboardCard><ModuleDashboardCard title="Processing Jobs">{(jobs.data ?? []).length === 0 ? <EmptyState title="No processing jobs yet" /> : <div className="space-y-3">{(jobs.data ?? []).map((job) => <div key={job.id} className="rounded-md border p-3"><div className="font-medium">{job.job_type}</div><div className="mt-2"><ProcessingStatusBadge value={job.status} /></div></div>)}</div>}</ModuleDashboardCard></div><AiPlaceholderPanel title="Future Multimodal AI">Future OCR, transcription, image understanding, diagram analysis, video summaries, and multimodal embeddings will build on this metadata model.</AiPlaceholderPanel></>}</div>;
}
