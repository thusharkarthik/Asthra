...
export function DeskBreadcrumbs({ items }: { items: Array<{ label: string; href?: string }> }) {
  const fullItems = [{ label: "Desk", href: "/desk" }, ...items];
  return (
    <nav aria-label="Desk breadcrumbs" className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
...
export function DeskBackLink({ href, label }: { href: string; label: string }) {
