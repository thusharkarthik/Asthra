...
export function PulseBreadcrumbs({ items }: { items: Array<{ label: string; href?: string }> }) {
  const fullItems = [{ label: "Pulse", href: "/pulse" }, ...items];
  return (
    <nav aria-label="Pulse breadcrumbs" className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
...
export function PulseBackLink({ href, label }: { href: string; label: string }) {
